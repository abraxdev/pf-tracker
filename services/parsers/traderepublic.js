...
async function parseTradeRepublic(filePath) {
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Pattern 1: "01 set" at start (date without year)
            const dateMatch = line.match(/^(\d{1,2}\s+\w{3})\s*$/);

            // Pattern 2: Only day number (for dates split across 3 lines like "30" \n "Mag" \n "2025")
            const dayOnlyMatch = line.match(/^(\d{1,2})\s*$/);
            ...
            if (dateMatch && i + 1 < lines.length) {
            ...
            } else if (dayOnlyMatch && i + 2 < lines.length) {
                // Handle date split across 3 lines: "30" \n "Mag" \n "2025"
                ...
                if (MONTHS_IT.includes(monthLine.toLowerCase())) {
                    // Check if third line starts with year
                    const yearMatch = yearLine.match(/^(\d{4})\s*(.*)/);
                    if (yearMatch) {
                        const year = yearMatch[1];
                        const restOfLine = yearMatch[2];
                        const dateStr = `${day} ${monthLine} ${year}`;
                        const parsedDate = parseDate(dateStr);
                        ...
                        if (amountMatches && amountMatches.length >= 1) {
                            ...
                            if (currentTransaction.amount_in || currentTransaction.amount_out) {
                                ...
                                console.log(`[TradeRepublic Parser] ✓ Added (3-line date): ${currentTransaction.date} - ${currentTransaction.type_raw} - ${finalDescription.substring(0, 30)}...`);
                            }

                            i += 2; // Skip month and year lines
                        } else {
                            // Format 2: Multiline (year on its own OR year + partial text, then more lines with amounts)
                            let j = i + 2; // Start from year line
                            let foundAmounts = false;
                            const textParts = [];
                            ...
                            while (j < lines.length && j < i + 15 && !foundAmounts) {
                                ...
                                // Skip the date, month, and year lines
                                if (j <= i + 2) {
                                    j++;
                                    continue;
                                }
